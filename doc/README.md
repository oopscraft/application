...
임베디드 웹서버 실행하여 서비스를 처리하는 구조를 기본으로 하여 WAR로 빌드하여 WAS(Web Application Server)에도 배포가 가능하도록 구성되는 구조로 구성된다.

## Database Access Strategy

데이터처리에 대한 기본 구조는 아래와 같다.

### Connection Pool (DBCP)

### Data Access Object (Integration with JPA[Spring-data,Hibernate] between MYBATIS)

정형화된 데이터 처리(정형화된 CRUD) 적합한 JPA와  비정형 데이터 처리(복합 조회,통계 등)에 적합한 MYBATIS를 동시에 사용하는 구조로 양측 Library의 장점을 모두 채용하는 구조를 채택한다.
...
### Transaction 처리 (Chained Transaction Manager)

Springframework의 @Transactional 어노테이션을 사용하여 처리한다.
